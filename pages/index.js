...
export default function Home() {
  ...
  const leadership = { 
    ...
    points: ["Organized workshops, seminars, and technical events.", "Collaborated with technical team for systems improvement."] 
  };
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Rakesh G - Data Science Student and Developer" />
      ...
      <div className="min-h-screen bg-[#030712] text-slate-200 font-sans selection:bg-violet-500/30 overflow-x-hidden">
        ...
        <section className="pt-56 pb-32 px-6 flex flex-col items-center text-center relative">
          ...
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="text-7xl md:text-9xl font-extrabold mb-6 tracking-tighter text-white">
            ...
            {projects.map((proj, i) => (
              <motion.div key={i} onClick={() => setSelectedProject(proj)} whileHover={{ y: -10 }} className="p-10 bg-[#0b0f1a] border border-white/5 rounded-[40px] cursor-pointer hover:border-violet-500/50 transition-all h-[440px] flex flex-col justify-between shadow-2xl group text-white">
                <div>
                  ...
                  <p className="text-slate-500 text-sm italic">Click for Case Study →</p>
                </div>
        ...
        <section className="max-w-5xl mx-auto px-6 py-24">
          ...
          <div className="p-10 rounded-[40px] bg-[#0b0f1a] border border-white/5 shadow-2xl text-white">
            <h3 className="text-2xl font-bold text-violet-400 tracking-tight flex items-center gap-4"><span>📢</span> {leadership.role}</h3>
            <p className="text-slate-400 font-medium mb-6 italic">{leadership.org} | {leadership.date}</p>

...
const Navbar = ({ setIsNavOpen }) => {
  return (
    <nav className="fixed top-0 w-full z-[100] px-6 py-6 flex justify-center">
      <div className="w-full max-w-4xl bg-[#0b0f1a]/80 backdrop-blur-md border border-white/10 rounded-2xl px-6 py-3 flex justify-between items-center shadow-2xl">
        ...
        <button onClick={() => setIsNavOpen(true)} className="md:hidden p-2 text-white">
          ...
          <div className="w-6 h-0.5 bg-white opacity-50"></div>
        </button>
      </div>
    </nav>
  );
...
export default function Home() {
  ...
  const projects = [
    { title: "Stock Market Prediction (LSTM)", tech: ["Python", "LSTM", "TensorFlow"], desc: "Designed and trained LSTM models for time series forecasting using historical market data." },
    ...
    { title: "Neural Pilot – Hill Climb AI", tech: ["Python", "Deep RL", "TensorFlow"], desc: "Developed a DRL agent for a physics-based control problem.", link: "https://github.com/Rakesh-developer28/Neural-Pilot-Hill-Climb-AI" },
  ...
  const experience = [
    { role: "Data Analyst Intern", company: "Tech Vaseegrah", date: "07/2024 – 08/2024", points: ["Analyzed and visualized business data using Python, Excel, and SQL.", "Built a billing and dashboard application using Flask."] },
    ...
    <div className="min-h-screen bg-[#030712] text-slate-200 font-sans selection:bg-violet-500/30 overflow-x-hidden">
      ...
      <AnimatePresence>
        {isNavOpen && (
          <>
            ...
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} className="fixed right-0 top-0 h-full w-72 bg-[#0b0f1a] border-l border-white/10 z-[120] p-8 shadow-2xl text-white">
          ...
          {projects.map((proj, i) => (
            <motion.div key={i} onClick={() => setSelectedProject(proj)} whileHover={{ y: -10 }} className="p-10 bg-[#0b0f1a] border border-white/5 rounded-[40px] cursor-pointer hover:border-violet-500/50 transition-all h-[440px] flex flex-col justify-between shadow-lg">
              <div>
                ...
                <h4 className="text-3xl font-bold mb-4 leading-tight">{proj.title}</h4>
                <p className="text-slate-500 text-sm italic line-clamp-3">Click to explore the detailed case study.</p>
              </div>
      ...
      <section id="contact" className="py-40 text-center relative border-t border-white/5">
        ...
        <div className="flex flex-wrap justify-center gap-8 px-6 text-white">
          <a href="mailto:rakesh28.dev@gmail.com" className="bg-[#0b0f1a] p-8 rounded-[32px] border border-white/5 hover:border-violet-500 transition-all flex items-center gap-4 shadow-xl">
             rakesh28.dev@gmail.com
          </a>
          <a href="https://www.linkedin.com/in/rakeshgdev" target="_blank" className="bg-white/5 px-10 py-8 rounded-[32px] border border-white/5 hover:text-violet-400 transition-all font-bold text-white">LinkedIn</a>
          <a href="https://github.com/Rakesh-developer28" target="_blank" className="bg-white/5 px-10 py-8 rounded-[32px] border border-white/5 hover:text-violet-400 transition-all font-bold text-white">GitHub</a>
        </div>
      </section>
